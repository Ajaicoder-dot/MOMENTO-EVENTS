...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['request_id'])) {
    ...
    if ($stmt->execute()) {
        ...
        // Changed JOIN condition from r.user_id = u.user_id to r.user_id = u.id
        $user_query = $conn->query("SELECT u.email FROM requests r JOIN users u ON r.user_id = u.id WHERE r.id = $request_id");
...
// Changed JOIN condition from r.user_id = u.user_id to r.user_id = u.id
$query = "SELECT r.*, u.username, u.email FROM requests r JOIN users u ON r.user_id = u.id WHERE 1=1";
                    ...
                    <div class="request-card">
                        ...
                        <div class="request-body">
                            ...
                            <div class="request-actions">
                                <button class="btn-respond" onclick="openResponseModal(<?php echo $request['id']; ?>, '<?php echo htmlspecialchars(addslashes($request['subject'])); ?>', '<?php echo htmlspecialchars($request['status']); ?>', `<?php echo htmlspecialchars($request['admin_response'] ?? ''); ?>`)">
